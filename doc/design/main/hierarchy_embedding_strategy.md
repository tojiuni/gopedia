...
* **운영 권장:** 상시 적용하되, 토큰 과증가를 막기 위해 canonical path(프로젝트/카테고리/파일/헤더)만 주입합니다.
...
4.  **ID 표준화**: `.gopedia` config의 `project_machine_id`를 루트로 `project_id`, `dataset_id`, `entity_id`, `file_id`, `chunk_id`를 생성해 3개 저장소를 동기화하세요.
5.  **Hybrid Query (고정)**: `TypeQL(구조 검색, 항상 실행) -> PostgreSQL/Parquet(수치 prefilter) -> Qdrant(의미 검색) -> PostgreSQL(운영 메타 결합)` 순서로 질의를 결합하십시오.
    * `TypeQL로 후보 file/chunk_id를 좁힌 뒤, Qdrant에서 벡터 검색하고, PostgreSQL 메타데이터(버전/권한/상태)를 조인해 최종 컨텍스트를 구성`

6.  **최소 토큰 전달**: 1차 응답에는 핵심 스니펫 + `l3_id`, `l2_id`, `l1_id`를 전달하고, 추가 근거 요청 시에만 parent 본문을 확장하세요.
