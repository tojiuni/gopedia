본 문서는 `markdown_strategy.md`의 sidecar 세부 규칙을 정의하는 **상세 부속 문서**입니다.
공통 정책(계층 분리, ID/링킹, 오케스트레이션)은 기준 문서를 우선 적용합니다.

- 기준 문서: `markdown_strategy.md`
- 프로세스 흐름: `markdown_process.md`
- 계층 검색 전략: `hierarchy_embedding_strategy.md`
...
> 공통 원칙 요약은 `markdown_strategy.md` 1장/6장을 참조하고, 본 문서는 sidecar 구현 규칙만 상세화합니다.
...
- Frontmatter는 운영 메타데이터(작성자, 수정일, 태그, 접근등급)만 유지합니다.
- 도메인 지식(관계, 수치, 스키마)은 sidecar로 분리합니다.
...
> 전체 embedding 정책은 `markdown_strategy.md` 3장을 기준으로 하고, 본 문서는 sidecar 전용 규칙만 상세화합니다.
...
### C. Sidecar Chunking 표준

- **JSON-LD chunk**
  - 기본 단위: `entity(@id)` 또는 `relation(triple)` 1개 = 1 chunk
  - 필수 메타: `dataset_id`, `entity_id`, `@type`, `path`, `ontology_version`, `admin_id`
- **Parquet/CSV chunk**
  - 기본 단위: 1 row = 1 chunk
  - 대용량은 핵심 컬럼 projection + row 요약 chunk를 우선 생성
  - 동일 스키마 데이터는 column-profile chunk(컬럼 의미/분포/단위)를 추가 생성
...
> 표준 산출물 목록의 기준은 `markdown_strategy.md` 8장을 따릅니다.

데이터셋 단위로 아래 파일 집합을 표준화합니다.

1. `{title}.md` - 사람 중심 문맥 (slug 권장)
2. `{dataset_id}.parquet` - 사실 데이터(SoT)
3. `{dataset_id}.schema.jsonld` - 의미/관계/타입
4. `{dataset_id}.profile.json` - row 수, null 비율, 갱신 시각

권장 매핑:
- `{title}.md` 내부에는 `dataset_id`를 명시하고, 참조는 `[[data-ref:<dataset_id>]]`로 통일
- 파일명 가독성(`title`)과 시스템 식별성(`dataset_id`)을 분리 운영
