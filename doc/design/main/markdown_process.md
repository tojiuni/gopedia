...
마크다운은 본문의 '맥락'을 담당하고, 사이드카 파일은 '데이터의 뼈대'를 담당하게 합니다.
...
* **Semantic Anchoring:** 마크다운 본문 내에서 사이드카의 특정 키(Key)를 참조할 때, `[[id:entity_name]]`과 같은 고유 식별자를 사용하여 본문과 정형 데이터 간의 연결 고리를 명시적으로 남깁니다.
...
* **ID 동기화 규칙:** `dataset_id` / `entity_id` / `chunk_id`를 공통 키로 사용하여 Markdown, JSON-LD, Parquet/CSV, DB 레코드를 일관되게 추적합니다.
...
* **Graph-Augmented Embedding:** 텍스트를 임베딩할 때, 해당 텍스트가 속한 GraphDB의 경로(Path) 정보를 텍스트 앞에 덧붙입니다.
...
2.  **병렬 검색:**
    * RDB에서 명확한 수치/범위 필터링.
    * GraphDB에서 관련 노드 및 주변 컨텍스트 추출.
    * VectorDB에서 유사한 텍스트 청크 추출.
