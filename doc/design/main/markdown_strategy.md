...
### 3-1. Chunk-aware embedding 단위

2장의 chunking 모델을 그대로 embedding 입력 단위로 사용합니다.

- **Markdown 계층**
  - L3를 기본 임베딩 단위로 사용 (`matched_content` 중심 검색)
  - L2/L1은 요약형 parent vector를 별도로 생성해 범위 축소용으로 사용
- **Sidecar 계층**
  - JSON-LD: `entity`/`relation` chunk를 semantic vector로 임베딩
  - Parquet/CSV: row 요약 chunk + column-profile chunk를 factual vector로 임베딩
- **공통 규칙**
  - 모든 vector payload에 `project_id`, `dataset_id`, `file_id`, `l1_id`, `l2_id`, `l3_id`, `section_type`를 넣어 retrieval 필터와 연결

### 3-2. 입력 템플릿 표준화

모든 임베딩 입력은 아래 순서로 canonicalize합니다.

1. `Path-to-Context` (축약 canonical path)
2. 구조 메타(`section_type`, `heading`, `dataset_id`)
3. 본문 텍스트(또는 sidecar row/entity 요약)

권장 형식:

`[Path: ...] [Type: ...] [IDs: ...] <content>`

이 표준을 사용하면 markdown/sidecar/chunk 유형이 달라도 벡터 공간 정합성을 유지할 수 있습니다.

### 3-3. Dual index + 후보 축소

- **Small index (L3/row/entity)**: 유사도 정밀 검색
- **Parent index (L2/L1/subgraph/summary)**: 검색 공간 축소
- 질의 흐름:
  1. Parent index로 범위 축소
  2. 축소 범위의 small index 상세 검색
  3. rerank 후 context compose

### 3-4. 모델/백엔드 운영 정책

- 임베딩 백엔드는 `openai`/`local` 모두 지원하되, 프로젝트별 메타 설정과 ingest 설정을 반드시 일치시킵니다.
- `pipeline_version`에 임베딩 모델/전처리 규칙을 기록하여 재인덱싱 기준을 명확히 유지합니다.
- 모델 변경 시에는 동일 데이터셋에 대해 A/B 평가를 수행해 회귀를 방지합니다.

### 3-5. 최소 토큰 전달 정책 (chunk 기반)

기본 응답은 식별자 중심으로 전달하고, 본문은 점진적으로 확장합니다.

- 1차 전달: 핵심 스니펫 + `l3_id`, `l2_id`, `l1_id` (+ sidecar는 `dataset_id`, `entity_id`)
- 2차 전달: 추가 근거 요청 시 parent chunk/L2 요약/row 원문 확장
- 목적: 근거 추적성 유지, 응답 지연 및 토큰 사용량 절감
