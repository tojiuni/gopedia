...
| Ingest returns **200** and stdout shows `doc_id`, but **Postgres is empty** (and stderr may show `RegisterProject failed … postgres not configured`) | The API loads a repo `.env` automatically (first `.env` found from cwd upward). Ensure `POSTGRES_USER` (and usually `POSTGRES_PASSWORD`, `POSTGRES_DB`) are set. If `POSTGRES_HOST` is omitted, it defaults to `127.0.0.1` for local runs. Without a real DB connection, Phloem still returns generated `doc_id` values but does **not** persist rows. |
| `GET /api/search` works but `GET /api/restore` returns 404 | Most likely a stale `./gopedia` binary (built before restore route updates). Rebuild and restart first: `GOTOOLCHAIN=auto go build -o gopedia ./cmd/gopedia` then `./gopedia server --addr 0.0.0.0:18787`. If `/api/restore` no longer returns 404 but still fails, check runtime env/DB connectivity (for example `POSTGRES_*`, `OPENAI_API_KEY`) and inspect API logs by `request_id`. |
...
| Postgres ping fails with `lookup postgres_db: no such host` (or similar) on the **host** | `.env` may set `POSTGRES_HOST=postgres_db` for Docker Compose; that hostname only resolves **inside** the compose network. When not running in a container, the API remaps `postgres_db` / `postgres` to `127.0.0.1` so the **published** Postgres port (e.g. 5432) is used. To force a dial host, set `GOPEDIA_POSTGRES_HOST`. |
| Qdrant / TypeDB warnings (`name resolver error`, `lookup qdrant` / `lookup typedb`) on the **host** | Same as Postgres: compose service names are remapped to `127.0.0.1` for Qdrant (`qdrant`) and TypeDB (`typedb`) when not in a container. Override with `GOPEDIA_QDRANT_HOST` / `GOPEDIA_TYPEDB_HOST` if needed. Ensure published ports match (`QDRANT_GRPC_PORT`, `TYPEDB_PORT`). |
